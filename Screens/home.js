...
import { SafeAreaView, View,StyleSheet, ActivityIndicator, FlatList, TouchableOpacity,Text, Image, ImageBackground } from 'react-native'
...
const Home = ({navigation}) => {
    ...
    useEffect(() => {
        ...
        _retrieveData().then(res =>{
            ...
            socket.on(res,(data)=>{
                ...
                // console.log(data)
                // setLoading(true)
                //    const filter = newData.filter((el) => el.productId !== data.productId)
                //     // _filterData();
                // if(data.type === "add"){
                //     setNewData(()=>([
                //         ...filter,
    ...
    const renderItem2 = ({item,index}) => {
        ...
        <View style={styles.newContainer}>
            <Image style={styles.newImage} source={{uri:item.productImage || item.image}} resizeMode='cover'/>
            <View style={{flexDirection:'column'}}>
            <Text style={styles.productName}>{item.productName || item.name} x {item.quantity}</Text>
    ...
    const image = { uri: "https://i.redd.it/qwd83nc4xxf41.jpg" };
    ...
    return (
           ...
           <SafeAreaView style={styles.container}>
               <ImageBackground source={image} resizeMode="cover" style={{flex:1}}>
            {
               ...
               <Header style={{backgroundColor:'#ea7774'}}>
          <Left>
            <TouchableOpacity transparent>
              <Icon name='power-off' color='white' size={20} onPress={_signOut} style={{marginLeft:10}}/>
            </TouchableOpacity>
          </Left>
          ...
          <Right>
            <TouchableOpacity transparent>
            <Icon name='sign-out-alt' color='white' size={20} onPress={_deactivate} style={{marginRight:10}}/>
            </TouchableOpacity>
          </Right>
        </Header>
      ...
      >
        <Text style={styles.mainText}>Choose Options 👇</Text>
            <TouchableOpacity style={styles.button} onPress={_deactivate2}>
                <Text style={styles.textButton}><Icon name='money-bill-wave' size={20}/>   Cash On Counter</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('WebViewScreen')}>
                <Text style={styles.textButton}><Icon name='paypal' size={20}/>   Cash through Paypal</Text>
            </TouchableOpacity>
             </RBSheet>
           </ImageBackground>
           </SafeAreaView>
